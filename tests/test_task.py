...
@pytest.mark.parametrize(
    ("session", "stim_time", "fix_time", "delay", "expected_modalities"),
    [
...
def test_derived_attributes(
    ...
    task = Task(NAME, session=session, stim_time=stim_time, fix_time=fix_time, delay=delay)
    assert task.modalities == expected_modalities
    assert task.n_inputs == len(expected_modalities) + 1  # add the start signal
    assert max(task.time) == stim_time + fix_time + delay
    assert len(task.time) == int((stim_time + fix_time + delay + task.dt) / task.dt)
