...
class GuidoTask:
    ...
    def scale_input(self, f):
        return 0.6 * (f - self.imin) / (self.imax - self.imin)
    ...
    def generate_trials(
        ...
        if catch_prob is not None:
            # non_catch_prob = (1 - catch_prob) / 3  # probability for generating 'v', 'a' and 'va' trials  # noqa: ERA001, E501 (commented out code, line too long)
            # p = [non_catch_prob] * 3 + [catch_prob]  # noqa: ERA001
            # modality = rng.choice(self.modalities, minibatch_size, p)  # noqa: ERA001
        ...
        inp_noise = (
            1 / alpha * np.sqrt(2 * alpha) * self.std_inp_noise * rng.normal(loc=0, scale=1, size=x.shape)
        )  #! altered
        # inp_noise = rng.normal(loc=0, scale=self.std_inp_noise, size=x.shape)  # noqa: ERA001 (commented out code)
